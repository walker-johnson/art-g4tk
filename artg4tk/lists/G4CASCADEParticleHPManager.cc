...
#include "Geant4/G4ParticleHPThreadLocalManager.hh"
#include "Geant4/G4HadronicException.hh"
#include "Geant4/G4Exception.hh"

G4CASCADEParticleHPManager* G4CASCADEParticleHPManager::instance = 0;

G4CASCADEParticleHPManager::G4CASCADEParticleHPManager()
: verboseLevel(1)
...
{
   messenger = new G4CASCADEParticleHPMessenger( this );
   ...
   if ( std::getenv( "G4NEUTRONHP_DO_NOT_ADJUST_FINAL_STATE" ) ||
	...
	<< "with the UI command: /process/had/particle_hp/do_not_adjust_final_state \n"
     ...
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_165", JustWarning, ed );
   }
   if ( std::getenv( "G4NEUTRONHP_USE_ONLY_PHOTONEVAPORATION" ) ) {
     ...
     ed << "Environmental variable G4NEUTRONHP_USE_ONLY_PHOTONEVAPORATION \n"
        ...
        << "in the next major release, Geant4 version 11.0";
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_166", JustWarning, ed );
   }
   if ( std::getenv( "G4NEUTRONHP_NEGLECT_DOPPLER" ) ||
	std::getenv( "G4PHP_NEGLECT_DOPPLER" ) ) {
     ...
     ed << "Environmental variables G4NEUTRONHP_NEGLECT_DOPPLER and G4PHP_NEGLECT_DOPPLER \n"
        ...
        << "in the next major release, Geant4 version 11.0";
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_167", JustWarning, ed );
   }
   if ( std::getenv( "G4NEUTRONHP_SKIP_MISSING_ISOTOPES" ) ) {
     ...
     ed << "Environmental variable G4NEUTRONHP_SKIP_MISSING_ISOTOPES \n"
        ...
        << "in the next major release, Geant4 version 11.0";
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_168", JustWarning, ed );
   }
   if ( std::getenv( "G4NEUTRONHP_PRODUCE_FISSION_FRAGMENTS" ) ) {
     ...
     ed << "Environmental variable G4NEUTRONHP_PRODUCE_FISSION_FRAGMENTS \n"
        ...
        << "in the next major release, Geant4 version 11.0";
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_169", JustWarning, ed );     
   }
   if ( std::getenv( "G4NEUTRON_HP_USE_WENDT_FISSION_MODEL" ) ) {
     ...
     ed << "Environmental variable G4NEUTRON_HP_USE_WENDT_FISSION_MODEL \n"
        ...
        << "in the next major release, Geant4 version 11.0";
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_170", JustWarning, ed );     
   }
   if ( std::getenv( "G4PHP_USE_NRESP71_MODEL" ) ) {
     ...
     ed << "Environmental variable G4PHP_USE_NRESP71_MODEL is valid but deprecated and \n"
        ...
        << "in the next major release, Geant4 version 11.0";
     G4Exception( "G4CASCADEParticleHPManager ", "HP_MAN_171", JustWarning, ed );     
   }
}
...
G4CASCADEParticleHPManager* G4CASCADEParticleHPManager::GetInstance()
{
  static G4CASCADEParticleHPManager manager;
  if (!instance)
  {
    instance = &manager;
  }
...
void G4CASCADEParticleHPManager::OpenReactionWhiteBoard()
{
   G4ParticleHPThreadLocalManager::GetInstance()->OpenReactionWhiteBoard();
}

G4ParticleHPReactionWhiteBoard* G4CASCADEParticleHPManager::GetReactionWhiteBoard()
{
   return G4ParticleHPThreadLocalManager::GetInstance()->GetReactionWhiteBoard();
}

void G4CASCADEParticleHPManager::CloseReactionWhiteBoard()
{
   G4ParticleHPThreadLocalManager::GetInstance()->CloseReactionWhiteBoard();
}

void G4CASCADEParticleHPManager::GetDataStream( G4String filename , std::istringstream& iss ) 
{
   G4String* data=0;
...
void G4CASCADEParticleHPManager::GetDataStream2( G4String filename , std::istringstream& iss ) 
{
...
void G4CASCADEParticleHPManager::SetVerboseLevel( G4int newValue )
{
...
void G4CASCADEParticleHPManager::register_data_file(G4String filename, G4String source)
{
   mDataEvaluation.insert( std::pair < G4String , G4String > ( filename , source ) );
}

void G4CASCADEParticleHPManager::DumpDataSource()
{
...
G4PhysicsTable* G4CASCADEParticleHPManager::GetInelasticCrossSections(const G4ParticleDefinition* particle )
{
...
void G4CASCADEParticleHPManager::RegisterInelasticCrossSections( const G4ParticleDefinition* particle, G4PhysicsTable* val )
{
   theInelasticCrossSections.insert( std::pair<const G4ParticleDefinition* , G4PhysicsTable* >( particle , val ) ); 
}

std::vector<G4ParticleHPChannelList*>* G4CASCADEParticleHPManager::GetInelasticFinalStates(const G4ParticleDefinition* particle)
{
...
void G4CASCADEParticleHPManager::RegisterInelasticFinalStates( const G4ParticleDefinition* particle , std::vector<G4ParticleHPChannelList*>* val )
{
...
void G4CASCADEParticleHPManager::DumpSetting() {
  G4cout << G4endl
         << "=======================================================" << G4endl
