...
#include "artg4tk/lists/G4RDAtomicTransitionManager.hh"
#include "G4RDFluoTransition.hh"

G4RDAtomicDeexcitation::G4RDAtomicDeexcitation():
  minGammaEnergy(100.*CLHEP::eV),
  minElectronEnergy(100.*CLHEP::eV),
  fAuger(false)
{}
...
{ 
  ...
  G4DynamicParticle* aParticle = NULL;
  G4int provShellId = 0;
  G4int counter = 0;
  ...
  do
	...
	// newShellId is given by GenerateFluorescence(...)
	{
      ...
      counter++;
      if (aParticle != NULL) {vectorOfParticles->push_back(aParticle);}
      else {provShellId = -2;}
    }
