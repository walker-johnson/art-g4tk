...
#include "artg4tk/lists/G4CASCADEParticleHPCaptureFS.hh"
...
#include "Geant4/G4Threading.hh"
  ...
  G4CASCADEParticleHPCapture::G4CASCADEParticleHPCapture()
  ...
  ,numEle(0)
  {
    ...
    std::cout << "****************************\n" << "Initializing G4CASCADEParticleHPCapture\n" <<  "****************************\n" << std::endl;
/*
...
//    G4cout <<"G4ParticleHPChannel constructed"<<G4endl;
    G4CASCADEParticleHPCaptureFS * theFS = new G4CASCADEParticleHPCaptureFS;
...
//  //    G4cout << "initializing theCapture "<<i<<" "<< numEle<<G4endl;
    ...
    delete theFS;
//    G4cout << "-------------------------------------------------"<<G4endl;
//    G4cout << "Leaving G4CASCADEParticleHPCapture::G4CASCADEParticleHPCapture"<<G4endl;
*/
  }
  ...
  G4HadFinalState * G4CASCADEParticleHPCapture::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {

    
    std::cout << "****************************\n" << "Executing G4CASCADEParticleHPCapture\n" <<  "****************************\n" << std::endl;

    //if ( numEle < (G4int)G4Element::GetNumberOfElements() ) addChannelForNewElement();

    G4CASCADEParticleHPManager::GetInstance()->OpenReactionWhiteBoard();
    if(std::getenv("NeutronHPCapture")) G4cout <<" ####### G4CASCADEParticleHPCapture called"<<G4endl;
    ...
    aNucleus.SetParameters(G4CASCADEParticleHPManager::GetInstance()->GetReactionWhiteBoard()->GetTargA(),G4CASCADEParticleHPManager::GetInstance()->GetReactionWhiteBoard()->GetTargZ());
    ...
    for ( G4int j = 0 ; j != iele ; j++ ) { 
       ...
       if ( target_isotope->GetN() == G4CASCADEParticleHPManager::GetInstance()->GetReactionWhiteBoard()->GetTargA() ) break; 
    }
    ...
    G4CASCADEParticleHPManager::GetInstance()->CloseReactionWhiteBoard();
    return result; 
  }

const std::pair<G4double, G4double> G4CASCADEParticleHPCapture::GetFatalEnergyCheckLevels() const
{
...
/*
void G4CASCADEParticleHPCapture::addChannelForNewElement()
{
   G4CASCADEParticleHPCaptureFS* theFS = new G4CASCADEParticleHPCaptureFS;
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) 
   {
      G4cout << "G4CASCADEParticleHPCapture Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
      theCapture.push_back( new G4ParticleHPChannel );
      (*theCapture[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
      (*theCapture[i]).Register(theFS);
   }
...
G4int G4CASCADEParticleHPCapture::GetVerboseLevel() const
{
   return G4CASCADEParticleHPManager::GetInstance()->GetVerboseLevel();
}
void G4CASCADEParticleHPCapture::SetVerboseLevel( G4int newValue ) 
{
   G4CASCADEParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}

void G4CASCADEParticleHPCapture::BuildPhysicsTable(const G4ParticleDefinition&)
{

   G4CASCADEParticleHPManager* hpmanager = G4CASCADEParticleHPManager::GetInstance();
...
void G4CASCADEParticleHPCapture::ModelDescription(std::ostream& outFile) const
{
